...
class RCRSManagementServer:
    ...
    def __init__(self, report_interval_seconds: int = 5):
    ...
    async def stream_reverse_connections(self, request_iterator, context):
        ...
        try:
            # Create a wrapper iterator that can catch and log decode errors
            async def debug_request_iterator():
                try:
                    async for request in request_iterator:
                        yield request
                except Exception as e:
                    logger.error(f"🔍 Raw decode error details:")
                    logger.error(f"   Exception type: {type(e).__name__}")
                    logger.error(f"   Exception message: {str(e)}")
                    logger.error(f"   Exception args: {e.args}")
                    
                    # Try to get more specific information about the DecodeError
                    if hasattr(e, 'message'):
                        logger.error(f"   DecodeError message: {e.message}")
                    if hasattr(e, 'details'):
                        logger.error(f"   DecodeError details: {e.details}")
            ...
            async for request in debug_request_iterator():
            ...
            logger.error(f"❌ Error in RCRS stream: {e} {traceback.format_exc()}")
        finally:
