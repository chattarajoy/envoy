...
Http::FilterHeadersStatus ReverseConnNotifierFilter::decodeHeaders(Http::RequestHeaderMap& headers,
                                                                   bool) {
  if (isReverseConnection(headers)) {
    interested_ = true;
...
void ReverseConnNotifierFilter::onDestroy() {
  if (interested_ && callbacks_ && callbacks_->requestHeaders()) {
    notify("terminated", *callbacks_->requestHeaders());
  }
}

void ReverseConnNotifierFilter::notify(absl::string_view event, const Http::RequestHeaderMap& headers) {
